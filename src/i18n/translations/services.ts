
export const servicesTranslations = {
  he: {
    services: {
      title: "השירותים שלנו",
      intro: "מגוון פתרונות מקצועיים המותאמים לצרכים שלך",
      description: "מגוון פתרונות מקצועיים בתחום הגיוס ופיתוח הקריירה",
      items: {
        careerCoaching: {
          title: "ייעוץ וליווי קריירה",
          description: "ליווי אישי מקצועי לפיתוח הקריירה שלך",
          fullDescription: `אנו מציעים שירות ייעוץ וליווי קריירה מקיף ומותאם אישית, המיועד לעזור לך להשיג את המטרות המקצועיות שלך.

הליווי כולל:
• אבחון תעסוקתי מעמיק לזיהוי חוזקות, כישורים ותחומי עניין
• בניית תכנית פעולה מפורטת להתפתחות מקצועית
• הכנה מקיפה לראיונות עבודה כולל סימולציות
• ייעוץ בנושא שכר ותנאי העסקה
• אסטרטגיות לפיתוח קריירה ארוך טווח
• תמיכה וליווי לאורך כל התהליך

הליווי ניתן על ידי עמית בקשי, מייסד ומנכ"ל החברה, עם ניסיון של למעלה מ-15 שנה בתחום הגיוס והייעוץ הקריירה.

השירות מתאים במיוחד ל:
• מחפשי עבודה בכל הרמות
• אנשים השוקלים שינוי קריירה
• מנהלים המעוניינים בקידום
• בוגרים טריים בתחילת דרכם המקצועית

כל תהליך מותאם אישית לצרכים והמטרות הספציפיות שלך.`
        },
        resumeWriting: {
          title: "כתיבת קורות חיים",
          description: "בניית קורות חיים מקצועיים ומותאמים אישית",
          fullDescription: `שירות כתיבת קורות חיים מקצועי המותאם לדרישות שוק העבודה העכשווי ולצרכים הספציפיים שלך.

השירות כולל:
• ניתוח מעמיק של הניסיון התעסוקתי והכישורים
• כתיבה מקצועית וממוקדת מטרה
• התאמה לדרישות התפקיד והתעשייה
• אופטימיזציה ל-ATS (מערכות מיון אוטומטיות)
• עיצוב מודרני ונקי
• גרסאות בעברית ובאנגלית

השירות מבוצע על ידי גל מושקוביץ, המנהל הטכנולוגי שלנו, המתמחה בהתאמת קורות חיים למערכות גיוס מתקדמות.

יתרונות:
• הגדלת הסיכויים לזימון לראיונות
• הבלטת ההישגים והיכולות המרכזיות
• התאמה לטרנדים העדכניים בתעשייה
• דגש על מילות מפתח רלוונטיות

הפורמט המתקדם שלנו הוכח כמגדיל משמעותית את סיכויי המועמדים להתקבל לתפקידים המבוקשים.`
        },
        personalBranding: {
          title: "מיתוג אישי ומקצועי",
          description: "בניית נוכחות דיגיטלית ומיתוג אישי חזק",
          fullDescription: `שירותי מיתוג אישי מקיפים לבניית נוכחות מקצועית חזקה ומשפיעה ברשת ומחוצה לה.

השירות כולל:
• בניית אסטרטגיית מיתוג אישי
• אופטימיזציה של פרופיל LinkedIn
• יצירת תוכן מקצועי ומעורר עניין
• ניהול מוניטין ברשת
• בניית נוכחות בפלטפורמות רלוונטיות
• ייעוץ לבניית רשת קשרים מקצועית

השירות מובל על ידי צוות המיתוג המקצועי שלנו בהובלת עמית בקשי.

תועלות מרכזיות:
• הגדלת החשיפה המקצועית
• משיכת הזדמנויות קריירה
• ביסוס מעמד כמומחה בתחום
• יצירת בידול מקצועי משמעותי

אנו מלווים אותך בבניית המותג האישי שלך בצורה אסטרטגית ומקצועית.`
        },
        recruitment: {
          title: "גיוס והשמה",
          description: "פתרונות גיוס והשמה מקיפים למעסיקים",
          fullDescription: `שירותי גיוס והשמה מתקדמים המותאמים לצרכי המעסיק ודרישות השוק.

השירות כולל:
• איתור מועמדים איכותיים
• סינון וראיונות ראשוניים
• בדיקות המלצות מקיפות
• ליווי תהליך הגיוס מתחילתו ועד סופו
• תמיכה בתהליכי הקליטה
• ייעוץ בנושאי שכר ותנאי העסקה

השירות מובל על ידי צוות הגיוס המקצועי שלנו בשיתוף עם עמית בקשי וגל מושקוביץ.

יתרונות ייחודיים:
• גישה למאגר מועמדים איכותי
• חיסכון בזמן ומשאבים
• התאמה מדויקת לדרישות התפקיד
• תהליך גיוס מקצועי ויעיל

אנו מתמחים במגוון תפקידים וענפים, עם דגש מיוחד על תחומי ההייטק, פיננסים, ושיווק.`
        }
      }
    }
  },
  en: {
    services: {
      title: "Our Services",
      intro: "Professional solutions tailored to your needs",
      description: "Comprehensive solutions in recruitment and career development",
      items: {
        careerCoaching: {
          title: "Career Coaching",
          description: "Professional career guidance and support",
          fullDescription: `We offer comprehensive and personalized career coaching services designed to help you achieve your professional goals.

Our coaching includes:
• In-depth career assessment to identify strengths, skills, and interests
• Detailed professional development action plan
• Comprehensive interview preparation including simulations
• Salary and benefits negotiation advice
• Long-term career development strategies
• Ongoing support throughout the process

Led by Amit Bakshi, our founder and CEO, with over 15 years of experience in recruitment and career counseling.

This service is particularly suitable for:
• Job seekers at all levels
• People considering career changes
• Managers looking for advancement
• Recent graduates starting their professional journey

Each process is tailored to your specific needs and goals.`
        },
        resumeWriting: {
          title: "Resume Writing",
          description: "Professional and personalized resume creation",
          fullDescription: `Professional resume writing service tailored to current job market requirements and your specific needs.

Service includes:
• In-depth analysis of work experience and skills
• Professional and goal-oriented writing
• Adaptation to job and industry requirements
• ATS optimization
• Modern and clean design
• Hebrew and English versions

Service is provided by Gal Moskowitz, our CTO, specializing in optimizing resumes for advanced recruitment systems.

Benefits:
• Increased chances for interview invitations
• Highlighting key achievements and capabilities
• Alignment with industry trends
• Focus on relevant keywords

Our advanced format has proven to significantly increase candidates' chances of getting their desired positions.`
        },
        personalBranding: {
          title: "Personal & Professional Branding",
          description: "Building strong digital presence and personal brand",
          fullDescription: `Comprehensive personal branding services for building a strong and influential professional presence online and offline.

Service includes:
• Personal branding strategy development
• LinkedIn profile optimization
• Professional content creation
• Online reputation management
• Presence building on relevant platforms
• Professional networking advice

Led by our professional branding team under the guidance of Amit Bakshi.

Key benefits:
• Increased professional exposure
• Attracting career opportunities
• Establishing expert status
• Creating meaningful professional differentiation

We guide you in building your personal brand strategically and professionally.`
        },
        recruitment: {
          title: "Recruitment & Placement",
          description: "Comprehensive recruitment solutions for employers",
          fullDescription: `Advanced recruitment and placement services tailored to employer needs and market requirements.

Service includes:
• Quality candidate sourcing
• Initial screening and interviews
• Comprehensive reference checks
• End-to-end recruitment process management
• Onboarding support
• Salary and benefits consulting

Led by our professional recruitment team in collaboration with Amit Bakshi and Gal Moskowitz.

Unique advantages:
• Access to quality candidate pool
• Time and resource savings
• Precise job requirement matching
• Professional and efficient recruitment process

We specialize in various roles and industries, with special focus on Hi-Tech, Finance, and Marketing.`
        }
      }
    }
  }
};
